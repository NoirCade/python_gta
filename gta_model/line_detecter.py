...
from input_keys import PressKey, ReleaseKey, W, A, S, D
...
def process_img(image):
    ...
    #중요!
    processed_img = cv2.Canny(processed_img, threshold1=50, threshold2=200) #하단임계값과 상단 임계값
    processed_img = cv2.GaussianBlur(processed_img, (5, 5), 0)
    
    # 원하는 영역을 만들고
    vertices = np.array([[200, 720], [450, 720], [450, 350], [880, 350], [880, 720], [1200,720],[1000,280],[250,280]], np.int32)
    ...
    lines = cv2.HoughLinesP(processed_img, 1, np.pi / 180, 100 ,35, 45) # https://opencv-python.readthedocs.io/en/latest/doc/25.imageHoughLineTransform/imageHoughLineTransform.html
    #위에 라인이 중요!
    # 직선의 기울기를 저장하는 변수 추가
    m1 = 0
    m2 = 0
...
while (True):
    ...
    screen = np.array(Image.open('./test_images/1.jpg'))
    ...
    #cv2.imshow('pygta5-7', new_screen)
    cv2.imshow('pygta5-7-2', cv2.cvtColor(new_screen, cv2.COLOR_BGR2RGB))
